...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (revealDateText) revealDateText.textContent = revealDate.toDateString();
  ...
  function updateCountdown() {
    ...
    if (diff <= 0) {
      ...
      if (!capsuleOpened && !revealStarted) startRevealSequence();
      return;
    }
  ...
  async function fetchEntriesData(db) {
    const paths = [appConfig.entriesPath, "capsuleEntries", "entries", "submissions", "responses"].filter(Boolean);
    ...
    return { path: paths[0] || "capsuleEntries", data: null };
  }

  async function loadCapsuleEntries() {
    ...
    try {
      ...
      const { data } = await fetchEntriesData(db);
      const rawEntries = data ? Object.entries(data).map(([id, value]) => ({ id, value: value || {} })) : [];

      entries = rawEntries.map(({ id, value }) => normalizeEntry(id, value)).sort((a, b) => b.sortTime - a.sortTime);
      if (entries.length && !selectedEntryId) selectedEntryId = entries[0].id;
  ...
  if (revealPassed) {
    if (shouldAnimateOnLoad) startRevealSequence();
    else openCapsule();
  }
