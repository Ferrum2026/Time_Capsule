...
function onFormSubmit(e) {
  ...
  if (!STRICT_NAME_PATTERN.test(name)) {
    throw new Error('Use name format: SURNAME_FIRST NAME_M.I');
  }
  if (!message) {
  ...
  let manifestUrl = '';
  try {
    manifestUrl = uploadJsonManifest(manifestPath, {
      displayName: name,
    ...
    Logger.log(`Storage manifest upload failed: ${storageError.message}`);
  }
